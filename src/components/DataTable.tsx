...
export const DataTable: React.FC<DataTableProps> = ({ leads, onApprove, emptyMessage = "Tidak ada data." }) => {
  if (leads.length === 0) {
    return (
  ...
  const getAccentClass = (category: string) => {
    ...
    if (cat.includes('resto') || cat.includes('cafe')) return 'row-accent-pink';
    if (cat.includes('sekolah') || cat.includes('edu')) return 'row-accent-purple';
    if (cat.includes('toko') || cat.includes('shop')) return 'row-accent-cyan';
              ...
              <td style={{ textAlign: 'right' }}>
                <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
                  ...
                  {onApprove && (
                    ...
                    >
                      <CheckSquare size={16} style={{ marginRight: '6px' }} /> Approve
                    </button>
                  )}
                </div>
              </td>
